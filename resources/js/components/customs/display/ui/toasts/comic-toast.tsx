...
export const comicToast = {
  ...
  async promise<T>(
    ...
    const id = toast.custom(
      () => (
        <div  className="comic-base comic-blue w-full max-w-lg min-w-md">
          <div className="group flex w-full items-center justify-center gap-3 p-2">
      ...
      .then((data) => {
        toast.custom(
          () => (
            ...
            >
              <div className="group flex w-full items-center justify-center gap-3 p-2">
                <CircleCheckIcon className="size-5 text-white group-hover:text-[#0051ff]" />
                <span className='text-md text-nowrap'>{messages.success(data)}</span>
              </div>
            </div>
          ),
          { id, unstyled: true },
